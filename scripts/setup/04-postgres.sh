...
# Role determines which config to load (default: primary)
ROLE="${ROLE:-primary}"

echo "=== PostgreSQL Installation (Role: $ROLE) ==="
...
# =============================================================================
# INITIALIZE CLUSTER (Primary only - standbys use pg_basebackup)
# =============================================================================
...
# Determine replication settings based on role
REPL_CONFIG=""
if [[ "$ROLE" == "primary" ]]; then
    REPL_CONFIG="
# === REPLICATION (Primary) ===
max_wal_senders = ${PG_MAX_WAL_SENDERS:-10}
max_replication_slots = ${PG_MAX_REPLICATION_SLOTS:-10}
synchronous_standby_names = '${PG_SYNCHRONOUS_STANDBY_NAMES:-}'
hot_standby = ${PG_HOT_STANDBY:-off}
"
else
    REPL_CONFIG="
# === REPLICATION (Standby) ===
max_wal_senders = ${PG_MAX_WAL_SENDERS:-10}
max_replication_slots = ${PG_MAX_REPLICATION_SLOTS:-10}
hot_standby = ${PG_HOT_STANDBY:-on}
hot_standby_feedback = ${PG_HOT_STANDBY_FEEDBACK:-on}
"
fi
...
# -----------------------------------------------------------------------------
# CLUSTER NAME (for replication identification)
# -----------------------------------------------------------------------------
...
if [[ "$ROLE" != "primary" ]]; then
    echo ""
    echo "NOTE: For standby setup, run 04b-replication.sh after primary is ready"
fi
