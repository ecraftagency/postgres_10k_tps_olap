#!/usr/bin/env python3
"""
Config Loader - Merge and Calculate

Merges config layers and calculates dynamic values:
1. base/*.env (golden facts)
2. hardware/{type}.env (hardware-specific)
3. workloads/{workload}.env (workload tuning)
4. Dynamic calculation (shared_buffers, hugepages, etc.)

Usage:
    python3 loader.py --hardware r8g.xlarge
    python3 loader.py --hardware r8g.xlarge --workload tpc-b
    python3 loader.py --hardware r8g.xlarge --workload tpc-b --output /tmp/config.env
"""
import argparse
import os
import sys
...
WORKLOADS_DIR = CONFIG_DIR / "workloads"
...
def load_config(
    ...
    """
    Load merged config from flat files.
    
    Priority (low to high):
    1. os.env (kernel/tcp)
    2. base.env (disk/raid)
    3. {role}.env (hardware specs + pg config)
    4. Dynamic calculation
    """
    config = {}
    
    # Layer 1: OS tuning
    config.update(load_env(CONFIG_DIR / "os.env"))
    
    # Layer 2: Base disk/raid tuning
    config.update(load_env(CONFIG_DIR / "base.env"))
    
    # Layer 3: Role-specific (primary or proxy)
    if auto_detect:
...
def export_env(config: Dict[str, str], output_path: Optional[Path] = None) -> str:
    ...
    lines = [
        ...
        f"# Hardware: {config.get('INSTANCE_TYPE', 'unknown')}",
        "",
    ]
    ...
    categories = {
        ...
        "DISK": [k for k in config if k.startswith("DATA_") or k.startswith("WAL_")],
        "POSTGRES": [k for k in config if k.startswith("PG_")],
        "BENCHMARK": [k for k in config if k.startswith("PGBENCH_") or k.startswith("FIO_")],
    }
...
def main():
    ...
    parser.add_argument("--role", "-r", default="primary", help="Role (primary, proxy)")
